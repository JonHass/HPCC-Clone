...
// generate data
        ...
        var timeArr = d3.range(0, TIMESTEP).map(t => {
            return {clusterc: []}
        });
    ...
    return {
        questions: [
            ...
            },
            {
                type: "html",
                name: "info",
                html: `<table><body><row><td><img src='images/${instance}_${timestep}_${task ? 'b' : 'a'}_${type ? 'b' : 'l'}.svg' /></td><td style='padding:20px'></td></row></body></table>`
            }
        ]
    }
}
    ...
    set.forEach(time => {
        d3.range(0, 2).forEach(ta => {
            d3.range(0, 2).forEach(ty => {
                question.push(generateobject(ins, time, ta, ty))
            })
        })
    })
});